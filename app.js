...
const state = {
  ...
  lightData: null,
  // --- Per-stage reward ---
  // rewardItem resolves to {src, label, source}; rewardReady is the in-flight
  // promise so a slow generation never blocks the child (see resolveRewardItem).
  rewardItem: null,
  rewardReady: null,
...
// ============================================================
// PER-STAGE REWARD
// See docs/plans/2026-07-16-stage-reward-design.md
// ============================================================

// Stages that pay out a collectible — the 5 real canvas stages, skipping the
// 'place-self' intro dummy. Keep in sync with TAB_ORDER.
const REWARD_STAGES = ['add-elements', 'color', 'light', 'sound-studio', 'animate'];

// Subjects follow the circumscribed-interests literature (South & Ozonoff 2005;
// Turner-Brown 2011; Anthony 2013): vehicles, dinosaurs and animals dominate,
// and symbolic interests (letters/numbers) are distinctively elevated in autism.
// PNGs are pre-generated by generate_reward_presets.py through the same Imagen
// pipeline as custom items, so both share one art style — and, being static,
// they still work on GitHub Pages where there is no server.
const REWARD_PRESETS = [
  { id: 'train',    label: 'Train',    src: 'assets/rewards/train.png' },
  { id: 'dinosaur', label: 'Dinosaur', src: 'assets/rewards/dinosaur.png' },
  { id: 'car',      label: 'Car',      src: 'assets/rewards/car.png' },
  { id: 'cat',      label: 'Cat',      src: 'assets/rewards/cat.png' },
  { id: 'numbers',  label: 'Numbers',  src: 'assets/rewards/numbers.png' },
  { id: 'letters',  label: 'Letters',  src: 'assets/rewards/letters.png' },
  { id: 'space',    label: 'Space',    src: 'assets/rewards/space.png' },
  { id: 'star',     label: 'Star',     src: 'assets/rewards/star.png' },
];
const DEFAULT_REWARD = REWARD_PRESETS.find(p => p.id === 'star');

// Bumped per caregiver pick so a superseded generation can't overwrite a newer one.
let rewardRequestId = 0;

// Custom text -> AI image, reusing /api/generate-sticker unchanged. event:'' asks
// for the neutral context: we want a standalone object, not one staged in a scene.
// Unavailable on GitHub Pages (API_BASE is '' there) — callers fall back.
async function generateRewardItem(text) {
  const resp = await fetch(API_BASE + '/api/generate-sticker', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: text, event: '' }),
  });
  if (!resp.ok) throw new Error('Generation failed');
  const data = await resp.json();
  return { src: data.image, label: text, source: 'generated' };
}

// Sets state.rewardReady to a promise that ALWAYS resolves to a usable item —
// never rejects, so no award path has to handle failure. Fire-and-forget: the
// caller does not await, letting generation run through the photo/event screens.
function setRewardItem({ preset, text }) {
  const status = document.getElementById('reward-status');

  if (preset) {
    state.rewardReady = Promise.resolve({ ...preset, source: 'preset' });
    if (status) status.textContent = '';
  } else {
    if (status) status.textContent = `Drawing a ${text}…`;
    state.rewardReady = generateRewardItem(text)
      .then(item => {
        if (status) status.textContent = `${text} ready!`;
        return item;
      })
      .catch(err => {
        // Offline / on Pages / model failure — degrade to the default star so a
        // collectible always flies and the bar never shows a hole.
        console.warn('Reward generation failed, falling back to default:', err);
        if (status) status.textContent = `Couldn’t draw that — using a star instead.`;
        return { ...DEFAULT_REWARD, source: 'fallback' };
      });
  }

  const myId = ++rewardRequestId;
  state.rewardReady.then(item => {
    // Ignore a late arrival that a newer pick or an award fallback has overtaken,
    // or the slots would end up showing two different things.
    if (myId !== rewardRequestId || state.rewardLocked) return;
    state.rewardItem = item;
    logEvent('reward_item_set', { label: item.label, source: item.source });
  });
}

function initRewardPicker() {
  ...
  REWARD_PRESETS.forEach(preset => {
    ...
    btn.dataset.rewardId = preset.id;
    btn.innerHTML = `<img class="reward-option-img" src="${preset.src}" alt="${preset.label}">
                     <span class="reward-option-label">${preset.label}</span>`;
    btn.addEventListener('click', () => {
      ...
      setRewardItem({ preset });
    });
    grid.appendChild(btn);
  });

  const input = document.getElementById('reward-custom-input');
  const go = document.getElementById('reward-custom-go');
  const submitCustom = () => {
  ...
  go?.addEventListener('click', submitCustom);
...
// Beat lengths of the celebration. The stage transition waits on the total, so
...
function spawnRewardSparkles(host, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    s.className = 'sparkle-particle';              // reuses the drum's particle CSS
    s.textContent = glyphs[Math.floor(Math.random() * glyphs.length)];
    const angle = (i / count) * 360 + Math.random() * 25 - 12;
    const distance = 150 + Math.random() * 110;
    const rad = angle * Math.PI / 180;
    s.style.setProperty('--dx', `${Math.cos(rad) * distance}px`);
    s.style.setProperty('--dy', `${Math.sin(rad) * distance}px`);
    s.style.fontSize = `${22 + Math.random() * 18}px`;
    host.appendChild(s);
  }
}

// Celebration: the item bursts in big at the canvas centre, holds so the child
// can see what they earned, then flies into its slot. Resolves when the slot has
// finished popping — callers gate the next stage on that, otherwise the new
// stage paints over the animation and the child never sees it.
//
// Uses the Web Animations API rather than class toggling: awaiting .finished
// keeps the three beats in sequence without hardcoded timer bookkeeping.
async function animateRewardToSlot(slot, item) {
  const canvasEl = document.getElementById('canvas-container') || document.getElementById('screen-canvas');
  ...
  try {
    ...
    ], { duration: REWARD_ANIM.popIn, easing: 'cubic-bezier(.34,1.56,.64,1)', fill: 'forwards' }).finished;

    // A second, smaller burst keeps the hold alive — the first one has faded out
    // by ~1.4s and the item would otherwise just sit there.
    spawnRewardSparkles(burst, 12);
    await wait(REWARD_ANIM.hold);

    // Read the slot rect late — the topbar may have reflowed during the hold.
    const to = slot.getBoundingClientRect();
    const dx = (to.left + to.width / 2) - cx;
    const dy = (to.top + to.height / 2) - cy;
    await flyer.animate([
      { transform: 'translate(-50%, -50%) scale(1) rotate(0deg)', opacity: 1 },
      { transform: `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px)) scale(0.24) rotate(16deg)`, opacity: 0.95 },
    ], { duration: REWARD_ANIM.fly, easing: 'cubic-bezier(.5,-0.12,.55,1)', fill: 'forwards' }).finished;
  } finally {
    flyer.remove();
    burst.remove();
  }
...
// Resolves the one item this session uses, and freezes it. Custom generation can
// still be in flight when the first stage ends; rather than freeze the child on a
// finished stage, fall back to the star after a short wait. The lock then keeps
// all five slots showing the same thing even if the generated image lands later.
function resolveSessionItem() {
  if (state.rewardItem) return Promise.resolve(state.rewardItem);
  const ready = state.rewardReady || Promise.resolve({ ...DEFAULT_REWARD, source: 'default' });
  return Promise.race([
    ready,
    wait(2500).then(() => ({ ...DEFAULT_REWARD, source: 'timeout' })),
  ]).then(item => {
    state.rewardLocked = true;
    if (!state.rewardItem) state.rewardItem = item;
    return state.rewardItem;
  });
}
...
function awardRewardFor(stageId) {
  ...
  state.rewardsAwarded.add(stageId);

  return resolveSessionItem().then(item => {
    const slot = document.querySelector(`.reward-slot[data-stage="${stageId}"]`);
    logEvent('reward_awarded', {
      stage: stageId,
      label: item.label,
      source: item.source,
      count: state.rewardsAwarded.size,
    });
...
// --- Animate done button ---
// (#btn-animate-done no longer exists — animate-bar is an empty placeholder and
// the drum finishes itself. Left as-is; the 'animate' reward is paid out on the
// live drum-complete path instead.)
...
function setupAnimatePhase() {
  ...
  function finishDrum() {
    ...
    // 5th collectible — a full bar is what unlocks the video, so wait for it to
    // actually land rather than racing a fixed timer against the animation.
    awardRewardFor('animate').then(() => setTimeout(() => goToPhase('processing'), 400));
  }
...
// {event}_{stem}.m4a convention (e.g. video/dental_voice.m4a). Stems should match the
...
const STEM_FILES_BY_EVENT = {
  ...
  dental:   { voice: 'video/dental_voice.m4a',  bgm: 'video/dental_bgm.m4a',  sfx: 'video/dental_sfx.m4a' },
  grocery:  { voice: 'video/grocery_voice.m4a', bgm: 'video/grocery_bgm.m4a', sfx: 'video/grocery_sfx.m4a' },
  dining:   { voice: 'video/dining_voice.m4a',  bgm: 'video/dining_bgm.m4a',  sfx: 'video/dining_sfx.m4a' },
};
...
function loadStems(ctx) {
  ...
  stemsLoading = Promise.all(STEM_KEYS.map(async (k) => {
    try {
      const res = await fetch(files[k] + '?v=3');
